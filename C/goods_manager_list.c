...
void init_list(GoodsList **L) {
    ...
    if ((fp = fopen(GOODS_FILE_NAME, "r")) == NULL) {
        if ((fp = fopen(GOODS_FILE_NAME, "w")) == NULL) {
            printf("文件%s为空。", GOODS_FILE_NAME);
            exit(1);
        }
    } else {
        while (!feof(fp)) {
            fscanf(fp, "%s", inf.goods_id);
            fscanf(fp, "\t%s", inf.goods_name);
            fscanf(fp, "\t%d", &inf.goods_price);
            fscanf(fp, "\t%s", inf.goods_discount);
            fscanf(fp, "\t%d", &inf.goods_amount);
            fscanf(fp, "\t%d\n", &inf.goods_remain);
...
bool delete_item(GoodsList *L, char* goods_id) {
	...
	p = L;
	//t = p->data;
	//strcpy(aim , t.goods_id);
	while(strcmp(p->data.goods_id , goods_id))
	{
		q = p;
		p = p->next;
		//t = p->data;
...
GoodsList* search_item(GoodsList *L, char* goods_id) {
	...
	p = q->next;
	t = p->data;
	while(t.goods_id != goods_id && p->next != NULL)
	{			
		q = q->next;
		p = p->next;
		t = p->data;
	...
	if(t.goods_id == goods_id)
		return p;
	else if(t.goods_id != goods_id && p->next == NULL)
		return NULL;
}
...
void output_one_item(GoodsList *p){
	...
	printf("ID:%s\t名称:%s\t价格:%3d\t折扣:%s\t数量:%d\t剩余:%d\n",
	       g.goods_id,g.goods_name,&g.goods_price,g.goods_discount,&g.goods_amount,&g.goods_remain//把你的代码添加在这里
		);
	}
}
...
int save_to_file(GoodsList *L) {
	...
	while(L != NULL)
	{
		fprintf(fp, "%s", L->data.goods_id);
        fprintf(fp, "\t%s", L->data.goods_name);
        fprintf(fp, "\t%d", L->data.goods_price);
        fprintf(fp, "\t%s", L->data.goods_discount);
        fprintf(fp, "\t%d", L->data.goods_amount);
        fprintf(fp, "\t%d\n", L->data.goods_remain);
		L = L->next;
		CurrentCnt++;
	}
